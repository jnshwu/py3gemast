...
class President(object):
    ...
    def _get_data(self,index):
        with open("/Users/jstrick/Documents/curr/py2harvard/StudentFiles/unix/py2harvard/DATA/presidents.txt") as pfile:
            ...
            for line in pfile:
                ...
                if int(fields[0]) == int(index):
                    ...
                    self._birth_date = self._mk_date(fields[3])
                    self._death_date = self._mk_date(fields[4])

                    self._birthplace = fields[5]
                    self._birth_state = fields[6]
    
                    self._ts_date = self._mk_date(fields[7])
                    self._te_date = self._mk_date(fields[8])
